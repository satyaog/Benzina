...
def test_file():
    ...
    with File(dataset_path) as f:
        input_trak = f.trak("bzna_input")
        assert input_trak is not None
        assert input_trak == f.trak("bzna_input\0")
        assert input_trak == f.trak(b"bzna_input")
        assert input_trak == f.trak(b"bzna_input\0")

    # Ownership of the file is held by disk_file
    disk_file = open(dataset_path, "rb")
    with File(disk_file):
        pass
    assert not disk_file.closed

    disk_file.close()
    assert disk_file.closed
    with File(disk_file) as f:
        assert f.closed
    ...
    # Force open the file
    f.open()
    # A new instance file handle is opened. The old one remains closed
    assert disk_file.closed
    assert not f.closed
    f.close()

    # Ownership of the file is held by f
    f = File(dataset_path)
    f.open()
    with Track(f, "bzna_input"):
        pass
    assert not f.closed

    f.close()
    assert f.closed
    with Track(f, "bzna_input") as t:
        assert t.file.closed

    f = File(dataset_path)
    f.open()
    t = Track(f, "bzna_input")
    # Force close the file
    t.close()
    assert f.closed
    assert t.file.closed
